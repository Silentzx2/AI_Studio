  ...
  return (
    ...
    >
      <div className="w-full max-w-xl max-w-[calc(100vw-1.5rem)] max-h-[90vh] rounded-2xl bg-[hsl(var(--surface-1))] border border-white/[0.08] shadow-2xl overflow-hidden flex flex-col">
        ...
        {/* Content */}
        <div className="p-4 sm:p-5 space-y-3 text-xs bg-[hsl(var(--surface-0))] flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-zinc-700">
          {bridges.map((b) => (
            <div key={b.id} className="p-3.5 rounded-xl bg-[hsl(var(--surface-2))] border border-white/[0.08] space-y-2.5">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-[hsl(var(--surface-0))] text-[hsl(var(--neon-cyan))] border border-white/[0.06]">Port {b.port}</span>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pt-1">
                ...
                <button
                  ...
                  className="self-start sm:self-auto flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[hsl(var(--surface-3))] hover:bg-[hsl(var(--surface-4))] text-zinc-200 hover:text-white border border-white/[0.08] transition-all cursor-pointer font-medium active:scale-95"
                >
                  {copiedApp === b.id ? <Check className="w-3.5 h-3.5 text-[hsl(var(--neon-green))]" /> : <Copy className="w-3.5 h-3.5" />}
        ...
        {/* Footer */}
        <div className="flex items-center justify-end px-4 sm:px-5 py-3.5 bg-[hsl(var(--surface-2))] border-t border-white/[0.08] flex-shrink-0">
          <button
            onClick={() => setIsDccBridgeOpen(false)}
            className="w-full sm:w-auto px-5 py-2 rounded-xl bg-primary hover:bg-primary/90 active:scale-95 text-primary-foreground font-extrabold text-xs shadow-md shadow-primary/20 transition-all cursor-pointer border border-white/20"
          >
            Done
          </button>
        </div>
