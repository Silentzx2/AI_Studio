...
export const RightPromptPanel: React.FC = () => {
  ...
  return (
    <div className="flex h-full flex-col bg-[#101115] text-xs">
      <div className="flex items-center justify-between border-b border-[#21242c] p-3">
        <div className="flex items-center gap-2">
          <FileJson className="h-4 w-4 text-[#f5c518]" />
          <span className="font-bold text-[#f3f4f6]">Prompt</span>
        </div>
        <div className="flex items-center gap-1">
          <button onClick={() => void load()} className="rounded-lg p-1.5 text-[#8e95a5] hover:bg-[#181a22] hover:text-[#f3f4f6]" title="Refresh prompt">
            <RefreshCw className={`h-3.5 w-3.5 ${loading ? 'animate-spin' : ''}`} />
          </button>
          <button onClick={() => void handleDelete()} disabled={!promptId || loading} className="rounded-lg p-1.5 text-[#8e95a5] hover:bg-[#29181b] hover:text-[#fca5a5] disabled:opacity-40" title="Delete history record">
