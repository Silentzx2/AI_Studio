    ...
    <div className="flex-1 p-6 flex flex-col gap-6 animate-fadeIn text-[hsl(var(--foreground))]" id="workspace-tab-panel">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4" id="workspace-stats-row">
        {[
          ...
          return (
            <div key={idx} className="bg-[hsl(var(--surface-1))] border border-[hsl(var(--border))] rounded-xl p-4 flex flex-col justify-between" id={`stat-card-${idx}`}>
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold text-[hsl(var(--muted-foreground))] uppercase tracking-wider">{stat.label}</span>
            ...
            {history.slice(0, 4).map((item) => (
              ...
              >
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <div className="flex items-center justify-between mt-2 pt-2 border-t border-[hsl(var(--border))] text-[9px] text-[hsl(var(--muted-foreground))] font-mono">
                    <span>🕒 {item.timestamp}</span>
                    <span className="text-[hsl(var(--primary))] group-hover:underline flex items-center gap-0.5 font-sans font-semibold">
              ...
              {[
                ...
                <div key={i} className="flex flex-col gap-1" id={`node-${i}`}>
                  <div className="flex justify-between items-center text-[10px]">
                    <span className="font-semibold text-[hsl(var(--muted-foreground))]">{node.name}</span>
                    <span className="font-mono text-[hsl(var(--muted-foreground))]">{node.load}</span>
                  </div>
                  <div className="flex items-center gap-1.5 mt-0.5">
                    <span className={`w-1.5 h-1.5 rounded-full ${node.color}`} />
                    <span className="text-[9px] text-[hsl(var(--muted-foreground))] font-medium">{node.status}</span>
                  </div>
                </div>
              ))}
            </div>
