...
- **Pre-Compiled 3D Binary Wheels**: Pre-installs official binary wheels (`spconv-cu124`, `torch-scatter`, `kiui`, `nvdiffrast`, `pytorch3d`) to bypass redundant native source builds.
- **Python 3.12 & PyTorch 2.5 Runtime Compatibility**: Automatically resolves PEP 585 GenericAlias schema inspection issues (`torch._library.infer_schema` type checking for `list[int]`) in `comfy_kitchen` eager ops under Python 3.12, and guards optional 3D imports in ComfyUI-3D-Pack nodes, preventing ComfyUI startup crashes.
- **Activity & System Logging**: Provides dedicated `/api/v1/system/log` endpoint receiving client-side UI telemetry from frontend `ActivityLogger`.
