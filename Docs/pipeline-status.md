...
2. **Workspace Shell (`features/workspace/WorkspaceShell.tsx`)**:
   - Main application shell with 3D viewport, tool panels, and inspector.
   - Supports workspaces: Generate, Texture, Remesh, Edit, Upscale, PBR, Animation, Segment.
   - Responsive layout with mobile drawer navigation.
   - Keyboard shortcuts (⌘1–⌘4, G/R/T/A/S) for tool navigation.

3. **State Management**:
   - Zustand stores (`useAppStore.ts`, `useWorkspace.ts`) for generation state, model selection, and UI panels.
...
|---|---|---|
| **System** | `backend/api/routers/system.py` | `/system/health`, `/system/info`, `/system/auth-status` |
| **File Upload** | `backend/api/routers/file_upload.py` | `/file-upload/image`, `/file-upload/mesh`, `/file-upload/{file_id}` |
...
| **Mesh Editing** | `backend/api/routers/mesh_editing.py` | `/mesh-editing/text-edit`, `/image-edit` |
| **Auto Rigging** | `backend/api/routers/auto_rigging.py` | `/auto-rigging` |
| **Mesh Segmentation** | `backend/api/routers/mesh_segmentation.py` | `/mesh-segmentation` |
| **Mesh Retopology** | `backend/api/routers/mesh_retopology.py` | `/mesh-retopology` |
| **Mesh UV Unwrapping** | `backend/api/routers/mesh_uv_unwrapping.py` | `/mesh-uv-unwrapping` |
...
| TRELLIS 2 | `backend/adapters/trellis2_adapter.py` | TRELLIS.2 | 23 GB | text, image | glb, obj |
