...
class ModalDeployer(Deployer):
    ...
    def deploy(self, spec: DeploySpec) -> DeployResult:
        ...
        env = {
            **os.environ,
            "CLAY_GPU_APP_NAME": spec.name,
            "CLAY_GPU_TYPE": spec.gpu,
            "CLAY_MODEL": spec.model,
            "CLAY_GPU_SCALEDOWN": str(spec.scaledown_window),
        }
        ...
        return DeployResult(
            ...
            detail=(
                "Deployed. Put this base URL in [gpu_backend].url."
                if url else
