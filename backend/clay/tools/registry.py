...
Param DSL: each param is either a shorthand string or a full dict spec.

  shorthand: ``"string"``, ``"integer"``, ``"number"``, ``"boolean"`` — append
             ``?`` to make optional (e.g. ``"integer?"``).
  full dict: {"type": ..., "enum": [...], "minimum"/"maximum": n,
              "description": str, "optional": bool, "default": any, "items": spec}

A tool callable has signature ``fn(ctx, args: dict) -> dict`` and returns a
result envelope (see ``result.py``). One registry drives both the agent
(OpenAI function-calling) and the MCP server, so behavior never drifts.
"""
...
def _spec_to_schema(spec: Any) -> dict:
    ...
    for key in ("enum", "minimum", "maximum", "description", "default", "examples"):
        if key in spec:
            schema[key] = spec[key]
    if spec["type"] == "array":
        schema["items"] = _spec_to_schema(spec.get("items", "string"))
    return schema
...
class ToolSpec:
    ...
    generates: bool = False  # hits the GPU backend (long-running / needs a deploy)
