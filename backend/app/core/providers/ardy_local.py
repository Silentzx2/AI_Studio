...
class ArdyLocalProvider(BaseProvider):
    ...
    async def generate(
        ...
        if not self.is_loaded or self.model is None:
            ...
            if not loaded:
                raise RuntimeError("Failed to load ARDY model into memory.")
        ...
        duration = float(getattr(request, "duration", 5.0) or 5.0)
        fps = getattr(self.model.motion_rep, "fps", 20.0)
        num_frames = int(duration * fps)

        if progress_callback:
            await progress_callback(15, "preparing", f"Preparing ARDY motion synthesis ({duration}s at {fps} fps)...")

        # 2. Diffusion steps & seed
        num_base_steps = int(self.model.diffusion.num_base_steps)
        ...
        # 3. Autoregressive diffusion generation
        if progress_callback:
        ...
        # 4. Optional motion post-processing
        if progress_callback:
            await progress_callback(80, "postprocessing", "Post-processing foot contacts & motion trajectories...")
